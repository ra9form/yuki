...
// Code generated by protoc-gen-goyuki. DO NOT EDIT.
// source: {{ .GetName }}

/*
Package {{ .GoPkg.Name }} is a self-registering gRPC and JSON+Swagger service definition.

It conforms to the github.com/ra9form/yuki/transport Service interface.
*/
package {{ .GoPkg.Name }}
import (
...
var (
	...
	pattern_goyuki_{{ $svc.GetName | goTypeName }}_{{ $m.GetName }}_{{ $b.Index }} = "{{ $b.PathTmpl.Template }}"

	pattern_goyuki_{{ $svc.GetName | goTypeName }}_{{ $m.GetName }}_{{ $b.Index }}_builder = func(in *{{ $m.RequestType.GoType $m.Service.File.GoPkg.Path | goTypeName }}) string {
		values := url.Values{}
		{{- if not (hasBody $b) }}
	...
	unmarshaler_goyuki_{{ $svc.GetName | goTypeName }}_{{ $m.GetName }}_{{ $b.Index }}_boundParams = {{ NewQueryParamFilter $b }}
	{{ end }}
	{{ end }}
	{{ end }}
)
...
var (
	...
	unmarshaler_goyuki_{{ $svc.GetName | goTypeName }}_{{ $m.GetName }}_{{ $b.Index }} = func(r *{{ pkg "http" }}Request) func(interface{})(error) {
		return func(rif interface{}) error {
			req := rif.(*{{$m.RequestType.GoType $m.Service.File.GoPkg.Path | goTypeName }})

			{{ if not (hasAsterisk $b.ExplicitParams) }}
			if err := {{ pkg "errors" }}Wrap({{ pkg "runtime" }}PopulateQueryParameters(req, r.URL.Query(), unmarshaler_goyuki_{{ $svc.GetName | goTypeName }}_{{ $m.GetName }}_{{ $b.Index }}_boundParams),"couldn't populate query parameters"); err != nil {
